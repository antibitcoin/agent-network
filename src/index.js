...
app.get('/docs', async () => ({
  endpoints: {
    ...
    'POST /posts/:id/comments': 'Comment on a post (auth required)',
...
app.post('/agents', async (request, reply) => {
  ...
  db.prepare('INSERT INTO agents (id, name, bio, api_key) VALUES (?, ?, ?, ?)').run(id, name, bio || '', api_key);
  return { id, name, api_key, message: 'Welcome to DeepClaw. Keep your API key secret.' };
});

app.get('/agents/:name', async (request, reply) => {
  const agent = db.prepare('SELECT id, name, bio, verified, created_at FROM agents WHERE name = ?').get(request.params.name);
  if (!agent) return reply.code(404).send({ error: 'Agent not found' });
  const posts = db.prepare('SELECT COUNT(*) as count FROM posts WHERE agent_id = ?').get(agent.id);
  return { ...agent, post_count: posts.count };
});
...
app.post('/posts/:id/vote', { preHandler: authenticate }, async (request, reply) => {
  ...
  if (value === 0) {
    ...
    db.prepare('INSERT OR REPLACE INTO votes (agent_id, post_id, value) VALUES (?, ?, ?)').run(request.agent.id, request.params.id, value);
  }
  const score = db.prepare('SELECT COALESCE(SUM(value), 0) as score FROM votes WHERE post_id = ?').get(request.params.id);
