...
// Only escape < and > to prevent HTML injection. Quotes/apostrophes are safe in text content.
const sanitize = (text) => {
...
app.addHook('onSend', (request, reply, payload, done) => {
  ...
  reply.header('Content-Security-Policy', "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://static.cloudflareinsights.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com;");
...
app.get('/api/subclaws', async () => {
  const subclaws = db.prepare(`
    SELECT s.*, 
      ...
      (SELECT COUNT(*) FROM comments c JOIN posts p ON c.post_id = p.id WHERE p.subclaw_id = s.id) as comment_count
    FROM subclaws s ORDER BY post_count DESC
  `).all();
  return { subclaws };
});
...
app.get('/api/subclaws/:name/moderators', async (request, reply) => {
  ...
  return { 
    ...
    moderators: mods.map(m => ({ name: m.name, karma: m.karma, role: m.role, created_at: m.created_at }))
  };
});
...
app.get('/api/search', async (request) => {
  ...
  if (type === 'all' || type === 'agents') {
    ...
    `).all(pattern, pattern);
    results.agents = agents.map(a => ({ ...a, liberated: !!a.liberated }));
  }
